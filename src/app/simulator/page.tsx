...
export default function SimulatorPage() {
  ...
  return (
    <div className="relative min-h-screen p-6 pb-24 bg-terra-space-950">
      ...
      <div className="space-y-3 max-w-md mx-auto">
        {scenarios.map((scenario, i) => (
          <motion.div key={scenario.id} initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
            <GlassCard
              className={`cursor-pointer ${selected === scenario.id ? 'border-terra-green-500/50' : ''}`}
      ...
      <AnimatePresence>
        {active && (
          <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 30 }} className="mt-8 max-w-md mx-auto">
            <GlassCard glow="aurora" className="text-center">
              <h3 className="text-lg font-display font-semibold mb-4">Your Impact in 1 Year</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <motion.div key={active.co2} initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-3xl font-mono font-bold text-terra-green-400">{active.co2}</motion.div>
                  <div className="text-xs text-terra-space-400 mt-1">kg CO₂ saved/year</div>
                </div>
                <div>
                  <motion.div key={active.trees} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1 }} className="text-3xl font-mono font-bold text-terra-green-400">🌳 {active.trees}</motion.div>
                  <div className="text-xs text-terra-space-400 mt-1">trees equivalent</div>
                </div>
              </div>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="mt-6 w-full py-3 rounded-xl bg-gradient-earth text-white font-semibold text-sm tracking-wide">
                START THIS CHANGE →
              </motion.button>
            </GlassCard>
          </motion.div>
