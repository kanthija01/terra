          ...
          {state === 'result' && (
            <motion.section key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4" role="region" aria-live="polite" aria-label="Scan results">
              <GlassCard className="text-center">
                ...
                <h2 className="font-display font-semibold text-lg">Beef Burger</h2>
                <div className="text-2xl font-mono font-bold text-terra-danger-500 mt-2" aria-label="6.6 kilograms CO2 per serving">6.6 kg CO₂</div>
                ...
                <motion.div key={alt.name} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.1 }}>
                  <GlassCard>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-terra-green-400 font-mono font-bold text-sm" aria-label={`Reduces impact by ${alt.saving} percent`}>↓{alt.saving}%</div>
                    </div>
                  </GlassCard>
                </motion.div>
              ))}
